...
// ── In-memory storage shim (localStorage blocked in artifact sandboxes) ──
const _store = {};
const safeStorage = {
  getItem: (k) => { try { return _store[k] ?? null; } catch { return null; } },
  setItem: (k, v) => { try { _store[k] = v; } catch {} },
  removeItem: (k) => { try { delete _store[k]; } catch {} },
};
...
// Cloudflare Worker proxies requests to Replicate — API key lives in the Worker, not here
const WORKER_URL = "https://runway-proxy.htg9872vnk.workers.dev";

async function generateVideo(scriptText) {
  // Extract a clean visual prompt from the script
  const prompt = scriptText.replace(/\[.*?\]/g, "").replace(/\n+/g, " ").trim().slice(0, 300);

  // Step 1: Start prediction using minimax/video-01 (fast, cheap ~$0.01/video)
  const createRes = await fetch(`${WORKER_URL}/predictions`, {
    method: "POST",
  ...
  if (!createRes.ok) {
    ...
    let errMsg = `Replicate error ${createRes.status}`;
    try { const j = JSON.parse(errBody); errMsg = j?.detail || j?.error || errMsg; } catch {}
  ...
  // Step 2: Poll until complete (up to 5 minutes)
  for (let i = 0; i < 60; i++) {
    await new Promise(r => setTimeout(r, 5000));
    const pollRes = await fetch(`${WORKER_URL}/predictions/${predictionId}`, {
      headers: { "Content-Type": "application/json" },
    });
...
function GeneratedResult({ title, content, onSave, showVideoGen = false, plan = "free", onDeductCredits }) {
  const [copied, setCopied] = useState(false);
  const [videoState, setVideoState] = useState("idle"); // idle | loading | done | error
  const [videoUrl, setVideoUrl] = useState(null);
  const [videoError, setVideoError] = useState(null);
  const [pollMsg, setPollMsg] = useState("");

  const canGenerateVideo = plan === "pro" || plan === "agency";
  ...
  const handleGenerateVideo = async () => {
    ...
    setVideoError(null);
    setVideoUrl(null);
    setPollMsg("Connecting to Replicate...");
    const msgs = [
    ...
    try {
      const url = await generateVideo(content);
      clearInterval(interval);
      if (onDeductCredits) onDeductCredits(100);
      setVideoUrl(url);
      setVideoState("done");
    } catch (e) {
      clearInterval(interval);
      setVideoError(e.message || "Unknown error — check your plan and try again");
      setVideoState("error");
    }
  };
      ...
      {showVideoGen && (
        <div style={{ marginTop: 20, borderTop: `1px solid ${COLORS.border}`, paddingTop: 18 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
            <div>
              <div style={{ fontWeight: 700, color: COLORS.text, fontSize: 14, marginBottom: 3 }}>🎬 Generate Video with AI</div>
              <div style={{ fontSize: 12, color: COLORS.textMuted }}>
                {canGenerateVideo ? "Turn this script into a short AI video reel — costs 100 credits" : "🔒 Video generation is a Pro & Agency feature"}
              </div>
            </div>
            {!canGenerateVideo && (
              <Badge color={COLORS.gold}>👑 Pro / Agency Only</Badge>
            )}
            {canGenerateVideo && videoState === "idle" && (
              <Btn onClick={handleGenerateVideo} style={{ background: "linear-gradient(135deg, #7c3aed, #4f46e5)" }}>
                🎬 Generate Video <span style={{ fontSize: 11, opacity: 0.8 }}>(-100 credits)</span>
              </Btn>
            )}
            {videoState === "loading" && (
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ width: 20, height: 20, border: `2px solid ${COLORS.accent}`, borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
                <span style={{ fontSize: 13, color: COLORS.textMuted }}>{pollMsg}</span>
              </div>
            )}
            {videoState === "done" && (
              <Btn variant="ghost" small onClick={() => { setVideoState("idle"); setVideoUrl(null); }}>🔄 Regenerate</Btn>
            )}
            {videoState === "error" && (
              <Btn variant="ghost" small onClick={() => setVideoState("idle")}>🔄 Try Again</Btn>
            )}
          </div>

          {videoState === "loading" && (
            <div style={{ marginTop: 16, background: "#141420", borderRadius: 10, padding: 24, textAlign: "center", border: `1px solid ${COLORS.border2}` }}>
              <div style={{ fontSize: 36, marginBottom: 12 }}>🎬</div>
              <div style={{ color: COLORS.text, fontWeight: 600, marginBottom: 6 }}>Creating your reel...</div>
              <div style={{ fontSize: 12, color: COLORS.textMuted }}>RunwayML is rendering your video. This usually takes 30–90 seconds.</div>
              <div style={{ marginTop: 16, height: 4, background: COLORS.border2, borderRadius: 2, overflow: "hidden" }}>
                <div style={{ height: "100%", background: `linear-gradient(90deg, ${COLORS.accent}, #4f46e5)`, borderRadius: 2, animation: "progress 3s ease-in-out infinite alternate", width: "60%" }} />
              </div>
            </div>
          )}

          {videoState === "error" && (
            <div style={{ marginTop: 14, background: "#ef444411", border: "1px solid #ef444433", borderRadius: 8, padding: 14 }}>
              <div style={{ color: COLORS.red, fontWeight: 600, marginBottom: 6, fontSize: 13 }}>⚠️ Video generation failed</div>
              <div style={{ color: "#ef444499", fontSize: 12, wordBreak: "break-word" }}>{videoError}</div>
              <div style={{ marginTop: 8, fontSize: 11, color: COLORS.textMuted }}>Check your RunwayML API key is valid and has available credits.</div>
            </div>
          )}
          ...
          {videoState === "done" && videoUrl && (
            <div style={{ marginTop: 16 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                <span style={{ color: COLORS.green, fontWeight: 600, fontSize: 13 }}>✅ Video Ready!</span>
              </div>
              <div style={{ display: "flex", justifyContent: "center" }}>
                <video
                  ...
                  loop
                  playsInline
                  style={{ maxWidth: 320, width: "100%", borderRadius: 12, border: `1px solid ${COLORS.border2}`, background: "#000" }}
                />
              </div>
              <div style={{ display: "flex", gap: 8, marginTop: 12, justifyContent: "center" }}>
                <a href={videoUrl} download="reelzai-video.mp4" target="_blank" rel="noreferrer" style={{ textDecoration: "none" }}>
                  <Btn variant="success" small>⬇️ Download Video</Btn>
                </a>
                <Btn variant="ghost" small onClick={() => { setVideoState("idle"); setVideoUrl(null); }}>🔄 New Video</Btn>
              </div>
            </div>
          )}
        </div>
...
function ScriptsPage({ onSave, plan = "free", setPage, onDeductCredits }) {
  ...
  const usedToday = () => { try { return parseInt(safeStorage.getItem(todayKey) || "0"); } catch { return 0; } };
  const bumpUsage = () => { try { safeStorage.setItem(todayKey, String(usedToday() + 1)); } catch {} };
  ...
  return (
    <div>
      ...
      {result && <GeneratedResult title="Generated Script" content={result} showVideoGen={true} plan={plan} onDeductCredits={onDeductCredits} onSave={() => onSave({ type: "Script", content: result, topic, date: new Date().toLocaleDateString() })} />}
    </div>
  );
}
...
function AuthScreen({ onAuth }) {
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      try {
        const users = JSON.parse(safeStorage.getItem("reelzai_users") || "{}");
        if (mode === "signup") {
          ...
          safeStorage.setItem("reelzai_users", JSON.stringify(users));
          const session = { email, name, credits: 50, plan: "free", newSignup: true };
          safeStorage.setItem("reelzai_session", JSON.stringify(session));
          onAuth(session);
        } else {
...
export default function App() {
  const [user, setUser] = useState(() => {
    try { return JSON.parse(safeStorage.getItem("reelzai_session")); } catch { return null; }
  });
  const [page, setPage] = useState("dashboard");
  const [credits, setCredits] = useState(() => {
    try {
      const session = JSON.parse(safeStorage.getItem("reelzai_session"));
  ...
  if (showPlanSelect) return <PlanSelectScreen onSelect={(selectedPlan) => {
    ...
    try {
      ...
      safeStorage.setItem("reelzai_session", JSON.stringify(session));
      const users = JSON.parse(safeStorage.getItem("reelzai_users") || "{}");
      if (session.email && users[session.email]) { users[session.email].plan = selectedPlan; users[session.email].credits = tier.credits; safeStorage.setItem("reelzai_users", JSON.stringify(users)); }
    } catch {}
  }} />;

  const handleLogout = () => {
    safeStorage.removeItem("reelzai_session");
    setUser(null);
    setPlan("free");
    setShowPlanSelect(false);
  };
  ...
  const pages = {
    ...
    scripts: <ScriptsPage onSave={handleSave} plan={plan} setPage={setPage} onDeductCredits={(amt) => setCredits(c => Math.max(0, c - amt))} />,
