...
const RUNWAY_API_KEY = "key_64e1a1d411664c34db5d6a0f856d586ff3ce1a758f5f9577293fd257a6e0aea8c9e509d64eab61bcfd5c4343aca0aa680110a69263f83462409e6e296fb69865";

// Cloudflare Worker proxy — forwards requests to Runway with proper CORS headers
const RUNWAY_BASE = "https://runway-proxy.htg9872vnk.workers.dev/v1";
...
async function generateRunwayVideo(scriptText) {
  // Extract a clean visual prompt from the script (first ~200 chars of main content)
  const prompt = scriptText.replace(/\[.*?\]/g, "").replace(/\n+/g, " ").trim().slice(0, 200);

  // Step 1: Create a text-to-video task
  const createRes = await runwayFetch("/text_to_video", {
    method: "POST",
    body: JSON.stringify({
      model: "gen4_turbo",
      promptText: prompt,
      ratio: "768:1344", // vertical/portrait for reels
      duration: 5,
    }),
  });
  ...
  if (!createRes.ok) {
    ...
    let errMsg = `RunwayML error ${createRes.status}`;
    try { const j = JSON.parse(errBody); errMsg = j?.error?.message || j?.message || j?.error || errMsg; } catch {}
    throw new Error(errMsg);
  }

  const task = await createRes.json();
  const taskId = task.id;
  if (!taskId) throw new Error("No task ID returned from RunwayML");

  // Step 2: Poll until complete (up to 5 minutes)
  for (let i = 0; i < 60; i++) {
    await new Promise(r => setTimeout(r, 5000));
    const pollRes = await runwayFetch(`/tasks/${taskId}`);
    if (!pollRes.ok) continue;
    const statusData = await pollRes.json();
    const s = (statusData.status || "").toUpperCase();
    if (s === "SUCCEEDED") {
      const url = statusData.output?.[0];
      if (!url) throw new Error("No video URL in RunwayML response");
      return url;
    }
    if (s === "FAILED") {
      throw new Error(statusData.failure || statusData.failureCode || "Video generation failed");
    }
...
function GeneratedResult({ title, content, onSave, showVideoGen = false }) {
  ...
  const handleGenerateVideo = async () => {
    ...
    setVideoError(null);
    setVideoUrl(null);
    setPollMsg("Sending to RunwayML...");
    const msgs = [
      "Sending to RunwayML...",
    ...
    try {
      const url = await generateRunwayVideo(content);
      clearInterval(interval);
      setVideoUrl(url);
      setVideoState("done");
    } catch (e) {
      clearInterval(interval);
      setVideoError(e.message || "Unknown error — check your RunwayML API key and credits");
      setVideoState("error");
    }
  };
      ...
      {showVideoGen && (
        <div style={{ marginTop: 20, borderTop: `1px solid ${COLORS.border}`, paddingTop: 18 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
            <div>
              <div style={{ fontWeight: 700, color: COLORS.text, fontSize: 14, marginBottom: 3 }}>🎬 Generate Video with RunwayML</div>
              <div style={{ fontSize: 12, color: COLORS.textMuted }}>Turn this script into a 5-second AI video reel (portrait, 9:16)</div>
            </div>
            {videoState === "idle" && (
              <Btn onClick={handleGenerateVideo} style={{ background: "linear-gradient(135deg, #7c3aed, #4f46e5)", border: "none" }}>
                🎬 Generate Video
              </Btn>
            )}
            {videoState === "loading" && (
...
function ScriptsPage({ onSave, plan = "free", setPage }) {
  ...
  return (
    <div>
      ...
      {result && <GeneratedResult title="Generated Script" content={result} showVideoGen={true} onSave={() => onSave({ type: "Script", content: result, topic, date: new Date().toLocaleDateString() })} />}
    </div>
  );
}
